...
config_args = {
    'training_config': {
        ...
        'save-dir': (
        'default_is_None', 'path to save training logs and model weights (defaults to logs/task/date/run/)'),
        'sweep-c': (0, ''),
        ...
        'repeats': (1, 'experiments to repeat')
    },
    'model_config': {
        ...
        'heads': (1, 'number of attention heads for graph attention networks, must be a divisor dim'),
        'n_heads': (1, 'number of attention heads for graph attention networks, must be a divisor dim'),
        'alpha': (0.2, 'alpha for leakyrelu in graph attention networks'),
        'agg_type': ('deg', 'attention type: attsparse, attdense, attpyg, deg'),
        'position': ('origin', 'origin or local'),
        'double-precision': ('0', 'whether to use double precision'),
        'beta': (1.0, 'lambda for curvature regularization'),
        'pretrained_embeddings': (None, 'path to pretrained embeddings (.npy file) for PubMed node classification'),
    },
    'data_config': {
        'dataset': ('cora', 'which dataset to use'),
        'edge_drop_ratio': (0.05, 'which dataset to use'),
        ...
        'sample-mode': (None, 'sample mode for data sampling (train/test/val)'),
        'data_root': ('./data', 'data root path'),
    }
}
import random
import os

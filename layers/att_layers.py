...
class DenseAtt(nn.Module):
    def __init__(self, in_features, dropout):
        ...
        self.linear = nn.Linear(2 * in_features, 1, bias=False)
    ...
    def forward(self, x, adj):
        ...
        support = torch.matmul(att_adj, x)
        return support
...
class SpGraphAttentionLayer(nn.Module):
    ...
    def forward(self, input, adj):
        N = input.size()[0]
        edge = adj._indices()
        input = torch.randn_like(input).to(input.device)
...
class HSpAttLayer(nn.Module):
    """
    Sparse version GAT layer, similar to https://arxiv.org/abs/1710.10903
    """

    def __init__(self, out_features, dropout):
        super(HSpAttLayer, self).__init__()
        self.out_features = out_features
        self.a = nn.Parameter(torch.zeros(size=(1, 2 * out_features)))
        nn.init.xavier_normal_(self.a.data, gain=1.414)
        self.dropout = nn.Dropout(dropout)
    ...
    def forward(self, h, adj):
        ...
        if h.is_cuda:
            ones = ones.cuda()
        e_rowsum = self.special_spmm(edge_index, edge_e, torch.Size([N, N]), ones)
        # e_rowsum: N x 1
        edge_e = self.dropout(edge_e)
        # edge_e: E
    ...
    def __repr__(self):
        return self.__class__.__name__ + ' (' + str(self.out_features) + ' -> ' + str(self.out_features) + ')'

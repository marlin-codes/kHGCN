...
# import layers.hypatt_layers as hyp_layers
import layers.hyp_layers as hyp_layers
from layers.layers import GraphConvolution, Linear, get_dim_act, GraphCurvConvolution, GraphSAGE, SGCConvolution
...
class GCN(Encoder):
    ...
    def __init__(self, c, args):
        ...
        for i in range(len(dims) - 1):
            ...
            # gc_layers.append(GraphCurvConvolution(in_dim, out_dim, args.dropout, act, args.bias))
...
class CurvGN(Encoder):
    """
    Graph Convolution Networks.
    """

    def __init__(self, c, args):
        super(CurvGN, self).__init__(c)
        assert args.num_layers > 0
        dims, acts = get_dim_act(args)
        gc_layers = []
        for i in range(len(dims) - 1):
...
class SAGE(Encoder):
    ...
    def __init__(self, c, args):
        ...
        for i in range(len(dims) - 1):
            in_dim, out_dim = dims[i], dims[i + 1]
            act = acts[i]
            gc_layers.append(GraphSAGE(in_dim, out_dim, args.dropout, act, args.bias))
        self.layers = nn.Sequential(*gc_layers)
        self.encode_graph = True
...
class HGCN(Encoder):
    ...
    def __init__(self, c, args):
        ...
        for i in range(len(dims) - 1):
            ...
            act = acts[i]
            hgc_layers.append(
                hyp_layers.HyperbolicGraphConvolution(self.manifold, in_dim, out_dim, c_in, c_out, args.dropout, act,
                                                      args.bias, heads=args.heads, agg_type=args.agg_type)
            )
...
class kHGCN(Encoder):
    """
    Curvature aware Hyperbolic-GCN.
    """

    def __init__(self, c, args):
        super(kHGCN, self).__init__(c)
        self.manifold = getattr(manifolds, args.manifold)()
        assert args.num_layers > 0
        dims, acts, self.curvatures = hyp_layers.get_dim_act_curv(args)
        self.curvatures.append(self.c)
        ...
        for i in range(len(dims) - 1):
            ...
            act = acts[i]
            hgc_layers.append(
                hyp_layers.CurvHGCN(self.manifold, in_dim, out_dim, c_in, c_out, args.dropout, act, args.bias,
                                    heads=args.heads, agg_type=args.agg_type, position=args.position)
            )
        self.layers = nn.Sequential(*hgc_layers)
        self.encode_graph = True
    ...
    def encode(self, x, adj):
        ...
        x_hyp = self.manifold.expmap0(x_tan, c=self.curvatures[0])
        x_hyp = self.manifold.proj(x_hyp, c=self.curvatures[0])
        return super(kHGCN, self).encode(x_hyp, adj)
    ...
    def __init__(self, c, args):
        ...
        if args.pretrained_embeddings is not None and args.num_layers > 0:
            ...
            for i in range(len(dims) - 1):
                ...
                act = acts[i]
                layers.append(Linear(in_dim, out_dim, args.dropout, act, args.bias))
        layers.append(Linear(args.feat_dim + weights.shape[1], weights.shape[1], args.dropout, nn.ReLU(), args.bias))
        self.layers = nn.Sequential(*layers)
        self.encode_graph = False
